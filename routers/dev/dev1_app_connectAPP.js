...
const deleteDEV1APPConfigById = (request, response) => {
    const { id } = request.body;
    var Id = parseInt(id);
    const deleteSqlQuery = 'DELETE FROM DEV1_CONFIG_DATA WHERE id = :id';
    run();
    async function run() {
        try{
            connection = await oracledb.getConnection(dbConfig);
        ...
        response.status(200).send(`Entry from Table DEV1_CONFIG_DATA deleted with ID: ${id}`)
    }
}
