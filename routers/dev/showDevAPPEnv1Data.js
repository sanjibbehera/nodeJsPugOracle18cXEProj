...
function loadDevAPPEnv1Page(){
    ...
    async function run() {
        try{
            ...
            Results = await connection.execute(`SELECT APPLICATION_NAME, SUB_APPLICATION_NAME, NVL(SOURCE_APPLICATION, 'N/A') SOURCE_APPLICATION, NVL(TARGET_APPLICATION, 'N/A') TARGET_APPLICATION, APPL_SERVICE_NAME, NVL(APPL_SERVICE_NAME_VERSION, 'N/A') APPL_SERVICE_NAME_VERSION, NVL(APPL_CONFIGURATION_URL, 'N/A') APPL_CONFIGURATION_URL, APPL_PACKAGE_NAME, APPL_SERVICE_TYPE, DECODE(APPL_SERVICE_STATUS, 'CRITICAL', 'RED', 'NORMAL', 'GREEN', 'HIGH', 'AMBER') APPL_SERVICE_STATUS FROM DEV1_CONFIG_DATA ORDER BY ID ASC`,
            [], // no bind variables
            {
            ...
            while ((row = await rs.getRow())) {
                ...
                env1appData.push(resultSetData);
            }      
    ...
    router.get('/', function(req, res) {
        ...
        app1DataList: env1appData}, function(err, html) {
            if (err) {
                //res.redirect('/404');
                res.status(404).end('error');
            } else {
                //console.log(env1appData);
...
setInterval(function(){
    loadDevAPPEnv1Page() // this will run after every 120 seconds
}, 120000);
